...
| **15** | **CEN (Scenario Pushbuttons)** | `OpenWebNet CEN Scenario Controls` / `WHO_15.pdf` | v1.0.0 (2008) | 🟢 | `event`, automations | Physical pushbuttons: `*15*BUTTON[#1/#2/#3]*WHERE##` for pressure, short release, long release and hold heartbeat. Buttons 00 to 31. |
...
| **25** | **CEN+ / Dry Contacts** | `OpenWebNet CEN+ Specification` / `WHO_25.pdf` | v1.0.0 (2011) | 🟢 | `event`, `binary_sensor` | CEN+ `*25*WHAT#PUSHBUTTON*2OBJECT##`: WHAT 21 short press (no release frame), 22 / 23 / 24 hold start / repeat / release, 25..28 rotary selector; pushbuttons 0 to 31, Objects 0 to 2047. Dry contacts and IR (3477, F428) share the WHO: `*25*31#1*WHERE##` ON, `32` OFF, `#0` for a state reply. |
